...
javascriptGenerator.forBlock["oop_getnoreturn_variable"] = function(block, generator) {
  const value_method = generator.valueToCode(block, 'NAME', Order.FUNCTION_CALL);
  let value_of = generator.valueToCode(block, 'VALUE', Order.ATOMIC);
  
  if (value_of.charAt(value_of.length - 1) === ".") {
    value_of = value_of.slice(0, -1);
  }

  return value_of + "." + value_method + ";\n";
}