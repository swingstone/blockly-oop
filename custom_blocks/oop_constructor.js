...
import {ObservableParameterModel, ObservableProcedureModel, ProcedureCreate} from "@blockly/block-shareable-procedures";

export const blocks = Blockly.common.createBlockDefinitionsFromJsonArray([
  // Block for a constructor.
  {
    type: "oop_constructor_def",
    message0: "constructor for %1 %2 %3",
    message1: "%1",
    args0: [
      {
    ...
    extensions: [
        "oop_constructor_def_get_def_mixin",
        "oop_constructor_def_var_mixin",
        "oop_constructor_def_update_shape_mixin",
        "oop_constructor_def_context_menu_mixin",
        "oop_constructor_def_onchange_mixin",
        "oop_constructor_def_validator_helper",
        "oop_constructor_get_caller_block_mixin",
        "oop_constructor_set_no_return_helper",
        "oop_constructor_get_class_name_helper",
    ],
    mutator: "oop_constructor_def_mutator",
  },
  // Block for creating a new object.
  {
    type: "oop_constructor_call",
    message0: "create new %1 %2",
    args0: [
      {
        type: "field_label",
        name: "NAME",
    ...
    output: null,
    colour: 35,
    tooltip: "Creates a new object of a class.",
    helpUrl: "",
    extensions: [
        "oop_constructor_caller_get_def_mixin",
        "oop_constructor_caller_var_mixin",
        "oop_constructor_caller_update_shape_mixin",
        "oop_caller_context_menu",
        "oop_caller_onchange_mixin",
        "oop_constructor_get_def_block_mixin",
    ],
    mutator: "oop_constructor_caller_mutator",
  },
]);
...
const procedureDefGetDefMixin = function () {
  const mixin = {
     ...
     *
     * @returns {null} The data model for this procedure block.
     */
    getProcedureModel() {
      return this.model_;
    },
    ...
    /**
     * True if this is a procedure definition block, false otherwise (i.e. it is a caller).
     *
     * @returns {boolean} True because this is a procedure definition block.
     */
    isProcedureDef() {
      return true;
    },
    ...
    /**
     * Disposes of the data model for this procedure block when the block is disposed.
     */
    destroy: function () {
      this.workspace.getProcedureModel().delete(this.getProcedureModel().getId());
    },
  };
  ...
  // Event cannot be fired from instantiation when deserializing or dragging from the flyout.
  // So make this consistent and never fire from instantiation.
...
Blockly.Extensions.register("oop_constructor_def_get_def_mixin", procedureDefGetDefMixin);

const procedureDefVarMixin = function () {
  const mixin = {
    /**
     * Notification that a variable is renaming.
     * If the ID matches one of this block's variable, rename it.
     *
     * @param oldId ID of variable to rename.
     * @param newId ID of the new variable.  May be the same type as the old variable.
     */
    renameVarById: function (oldId, newId) {
      const oldVar = this.workspace.getVariableById(oldId);
      const model = this.getProcedureModel();
      const index = model.getParameters().findIndex((p) => p.getVariableModel() === oldVar);
     ...
     * If the variable is in use on this block, rerender to show the new name.
     *
     * @param variable The variable being renamed.
     */
    updateVarName: function (variable) {
      const containsVar = this.getProcedureModel().getParameters().some((p) => p.getVariableModel() === variable);
      if (containsVar) {
...
Blockly.Extensions.register("oop_constructor_def_var_mixin", procedureDefVarMixin);
...
const procedureDefUpdateShapeMixin = {
  ...
  /**
   * Updates the parameters field to reflect the parameters in the procedure model.
   * @private
   */
  updateParameters_: function () {
    const params = this.getProcedureModel().getParameters().map((p) => p.getName());
  ...
  /**
   * Updates the parameter blocks in the mutator (if it is open) to reflect the state of the procedure model.
   * @private
   */
...
Blockly.Extensions.registerMixin("oop_constructor_def_update_shape_mixin", procedureDefUpdateShapeMixin);
...
Blockly.Extensions.register("oop_constructor_def_validator_helper", procedureDefValidatorHelper);
...
const procedureDefMutator = {
  ...
  mutationToDom: function () {
    ...
    // Save whether the statement inputs is visible.
   ...
   *
   * @param doFullSerialization Tells the block if it should serialize its entire state
   *      (including data stored in the backing procedure model).  Used for copy-paste.
   * @returns {any} The state of this block, e.g. the parameters and statements.
   */
   ...
   *
   * @param state The state to apply to this block, e.g. the parameters and statements.
   */
  ...
  compose: function (containerBlock) {
    // Note that only one of these four things can actually occur for any given composition,
    // because the user can only drag blocks around so quickly.  So we can use that when making
    // assumptions inside the definitions of these sub procedures.
  ...
  /**
   * Deletes any parameter from the procedure model that do not have associated parameter blocks in the mutator.
   *
  ...
  /**
   * Renames any parameters in the procedure model whose associated parameter blocks have been renamed.
   *
   * @param containerBlock Root block in the mutator.
   * @private
   */
  ...
  /**
   * Adds new parameter to the procedure model for any new procedure parameter blocks.
   *
   * @param containerBlock Root block in the mutator.
   * @private
   */
...
Blockly.Extensions.registerMutator("oop_constructor_def_mutator", procedureDefMutator, undefined, ["procedures_mutatorarg"]);
...
const procedureDefContextMenuMixin = {
  ...
  customContextMenu: function (options) {
    ...
    // Add option to create a caller.
    options.push({
      enable: true,
      text: Blockly.Msg["PROCEDURES_CREATE_DO"].replace("%1", this.getFieldValue("NAME")),
    ...
    for (const param of params) {
      ...
      options.push({
        enable: true,
        text: Blockly.Msg["VARIABLES_SET_CREATE_GET"].replace("%1", argVar.name),
        callback: Blockly.ContextMenu.callbackFactory(this, argXmlBlock),
      });
    }
  },
};
Blockly.Extensions.registerMixin("oop_constructor_def_context_menu_mixin", procedureDefContextMenuMixin);
...
Blockly.Extensions.registerMixin("oop_constructor_def_onchange_mixin", procedureDefOnChangeMixin);

const procedureDefNoReturnGetCallerBlockMixin = {
  callType_: "oop_constructor_call",
};
Blockly.Extensions.registerMixin("oop_constructor_get_caller_block_mixin", procedureDefNoReturnGetCallerBlockMixin);

const procedureDefSetNoReturnHelper = function () {
  this.getProcedureModel().setReturnTypes(null);
};
Blockly.Extensions.register("oop_constructor_def_set_no_return_helper", procedureDefSetNoReturnHelper);
...
const procedureCallerGetDefMixin = function () {
  const mixin = {
     ...
     *
     * @returns {null} The procedure model associated with this block.
     */
    getProcedureModel() {
      return this.model_;
    },
     ...
     * @returns {IProcedureModel|null} The procedure model that was found.
     * @private
     */
    findProcedureModel_(name, params = []) {
      const workspace = this.getTargetWorkspace_();
      const model = workspace.getProcedureMap().getProcedures().find((proc) => proc.getName() === name);
      ...
      const hasMatchingParams = model.getParameters().every((p, i) => p.getName() === params[i]);
    ...
    /**
     * Returns the main workspace (i.e. not the flyout workspace) associated with the block.
     *
     * @returns {WorkspaceSvg|*} The main workspace (i.e. not the flyout workspace) associated with the block.
     * @private
     */
    getTargetWorkspace_() {
     ...
     * True if this is a procedure definition block, false otherwise (i.e. it is a caller).
     *
     * @returns {boolean} False because this is not a procedure definition block.
     */
    isProcedureDef() {
      return false;
    },
...
// Using register instead of registerMixin to avoid triggering warnings about overriding built-ins.
Blockly.Extensions.register("oop_constructor_caller_get_def_mixin", procedureCallerGetDefMixin);

const procedureCallerVarMixin = function () {
  const mixin = {
    /**
     * Notification that a variable is renaming but keeping the same ID.  If the variable is in use
     * on this block, rerender to show the new name.
     *
     * @param variable The variable being renamed.
     */
    updateVarName: function (variable) {
...
Blockly.Extensions.register("oop_constructor_caller_var_mixin", procedureCallerVarMixin);
...
const procedureCallerMutator = {
   ...
   * Returns the state of this block as a JSON serializable object.
   *
   * @returns {any} The state of this block, i.e. params and procedure name.
   */
  saveExtraState: function () {
    const state = Object.create(null);
    const model = this.getProcedureModel();
    if (!model) {
      // We reached here because we've deserialized a caller into a workspace where its model did not already exist (no
      // procedures array in the JSON, and deserialized before any definition block), and are reserializing it before
      // the event delay has elapsed and change listeners have run.  (If they had run, we would have found or created
      // a model).  Just reserialize any deserialized state.  Nothing should have happened in between to change it.
   ...
   * Applies the given state to this block.
   *
   * @param state The state to apply to this block, i.e. the params and procedure names.
   */
  loadExtraState: function (state) {
...
Blockly.Extensions.registerMutator("oop_constructor_caller_mutator", procedureCallerMutator);
...
const procedureCallerUpdateShapeMixin = {
  ...
  /**
   * Updates the shape of the block to reflect the state of the data model.
   */
  doProcedureUpdate: function () {
  ...
  /**
   * Updates the enabled state of this block to match the state of the data model.
   * @private
   */
  ...
  /**
   * Updates the parameter fields / inputs of this block to match the state of the data model.
   * @private
   */
  updateParameters_: function () {
    this.syncArgsMap_();
  ...
  /**
   * Makes sure that if we are updating the parameters before any move events have happened, the args map records
   * entries from the array, since blocks can be disconnected temporarily during mutation (which triggers this method).
   * @private
   */
  syncArgsMap_: function () {
    // We look at the prevParams array because the current state of the block matches the params, not the new params state.
  ...
  /**
   * Saves a map of parameter IDs to target blocks attached to the inputs of this caller block.
   * @private
   */
  ...
  /**
   * Adds or remove the parameter label to match the state of the data model.
   * @private
   */
   ...
   *
   * @param params The params to add to the block, or null to use the params defined in the procedure model.
   * @private
   */
  ...
  /**
   * Reattaches blocks to this block's inputs based on the data saved in the argsMap_.
   * @private
   */
  ...
  /**
   * Notification that a procedure is renaming.  If the name matches this block's procedure, rename it.
   *
...
Blockly.Extensions.registerMixin("oop_constructor_caller_update_shape_mixin", procedureCallerUpdateShapeMixin);

const procedureCallerOnChangeMixin = {
  /**
   * Procedure calls cannot exist without the corresponding procedure definition.
  ...
  onchange: function (event) {
    ...
    // Look for the case where a procedure call was created (usually through paste) and there is no matching definition
    // block with the correct signature.
    ...
    if (!def) {
      ...
      this.model_ = this.createDef_(this.getFieldValue("NAME"), this.paramsFromSerializedState_);
      Blockly.Events.setGroup(false);
    }
    if (!this.getProcedureModel()) {
      // We have a def, but no reference to its model.
      this.model_ = this.findProcedureModel_(this.getFieldValue("NAME"), this.paramsFromSerializedState_);
    }
    this.initBlockWithProcedureModel_();
  },
   ...
   * @returns {false} True if the given event is a paste event for this block.
   * @private
   */
  ...
  /**
   * Returns true if the given block matches the definition of this caller block.
   *
   * @param defBlock The definition block to check against.
   * @returns {boolean} Whether the def block matches or not.
   * @private
   */
   ...
   * @returns {*} The procedure model associated with the new procedure definition block.
   * @private
   */
  createDef_(name, params = []) {
    console.log("Called createDef_");
...
Blockly.Extensions.registerMixin("oop_constructor_caller_onchange_mixin", procedureCallerOnChangeMixin);

const procedureCallerContextMenuMixin = {
  /**
   * Add menu option to find the definition block for this call.
   *
   * @param options List of the menu options to add to.
   */
  customContextMenu: function (options) {
    if (!this.workspace.isMovable()) {
      // If we center on the block and the workspace isn't movable, we could lose blocks at the edges of the workspace.
      return;
    }
    const name = this.getFieldValue("NAME");
...
Blockly.Extensions.registerMixin("oop_constructor_caller_context_menu_mixin", procedureCallerContextMenuMixin);

const procedureCallerNoReturnGetDefBlockMixin = {
  hasReturn_: false,
  defType_: "oop_constructor_def",
};
Blockly.Extensions.registerMixin("oop_constructor_get_def_block_mixin", procedureCallerNoReturnGetDefBlockMixin);

Blockly.Extensions.register("oop_constructor_get_class_name_helper", function () {
  if (this.isInFlyout) {
    this.setFieldValue(
